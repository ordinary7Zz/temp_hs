...
from damage_models import DamageParameter, CSVExporter, JSONExporter
...
class ExportWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            items: List[DamageParameter] = repo.get_all()
            ...
            if self.fmt == "csv":
                filename = os.path.join(self.out_dir, f"DamageParameter_{ts}.csv")
                ...
                filename = os.path.join(self.out_dir, f"DamageParameter_{ts}.json")
...
class DamageParameterExportDialog(QDialog):
    """毁伤参数导出对话框"""

    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("导出毁伤参数")
        self.setModal(True)
        self.resize(500, 200)
...
if __name__ == "__main__":
    ...
    dialog = DamageParameterExportDialog()
    dialog.exec()
